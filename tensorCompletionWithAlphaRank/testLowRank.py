...
def main(unused_arg):
  ...
  for i in range(num_players):
    ...
    random_factors = [tl.tensor(np.random.uniform(low=-5, high=5, size=(s,5))) for s in (11,11,11)] # random_sample((s, 5))
  ...
  for i in range(len(payoff_missing_tensors)):
    ...
    tncp1 = TNCP(payoff_missing_tensors[i], omega, rank=r, tol=1e-20, max_iter=5000) # 
    tncp1.run()
    recoerved_result = tncp1.X
    print("shape of recoerved_result:", recoerved_result.shape)
    ...
    print ('\n', 'The Relative Error of the cp_als are:', ReErr21)
  ...
  print(completed_payoff_tables[0])
  ...
  print("RMSE:", np.sqrt(np.mean((pi_est-pi)**2)))
  print(len(pi))
